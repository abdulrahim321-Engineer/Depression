...
print("\n" + "=" * 60)
print("  Running ANOVA + Boruta 2-View (TF+MI) Pipeline")
print("=" * 60)
...
for fold, (tri, tei) in enumerate(cv.split(uniq, sl, groups=uniq)):
    ...
    # --- Feature Selection (ANOVA + Boruta) ---
    print(f"\nFold {fold+1}:")
    ...
    try:
        ...
        # If too strict (selected 0), fallback to Confirmed + Tentative
        if np.sum(selected_mask) == 0:
            selected_mask = boruta_sel.support_ | boruta_sel.support_weak_
            
        # If still 0, fallback to top 15 features by rank
        if np.sum(selected_mask) == 0:
        ...
        print("  [WARNING] 'boruta' package not found. Run 'pip install boruta' to use it.")
        print("  [WARNING] Falling back to standard top 128 ANOVA features...")
        ...
        print(f"  [WARNING] Boruta execution failed ({ex}). Falling back to top 128 ANOVA features...")
        anova_fb = SelectKBest(score_func=f_classif, k=min(128, Xtf_tr_r.shape[1]))
        Xtf_tr = anova_fb.fit_transform(Xtf_tr_r, y_tr)
        Xtf_te = anova_fb.transform(Xtf_te_r)
...
# Final Summary Table
print(f"\n{'='*70}\n  SUMMARY: ANOVA + Boruta Feature Selection Experiment\n{'='*70}")
...
ax.set_title("ANOVA + Boruta 2-View Classifier & Ensemble Performance")
ax.set_xticks(x)
ax.set_xticklabels(all_names)
ax.set_ylim(0, 1.05)
ax.legend(loc="lower right")
plt.tight_layout()
plot_file = os.path.join(OUT_DIR, "Kaggle_2view_Boruta_Ensemble.png")
...
cm_plot_file = os.path.join(OUT_DIR, "Kaggle_2view_Boruta_KNN_Confusion_Matrix.png")
plt.tight_layout()
...
ax.legend(loc="lower right")
roc_plot_file = os.path.join(OUT_DIR, "Kaggle_2view_Boruta_ROC_Curves.png")
plt.tight_layout()
