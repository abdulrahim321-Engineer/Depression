...
print("\n" + "=" * 60)
print("  Running ANOVA + Boruta + MLP (PyTorch) Pipeline")
print("=" * 60)
...
for fold, (tri, tei) in enumerate(cv.split(uniq, sl, groups=uniq)):
    ...
    # Evaluate the Soft Voting Ensemble (SVC + KNN + PyTorchMLP)
...
# Final Summary Table
print(f"\n{'='*70}\n  SUMMARY: ANOVA + Boruta + MLP (PyTorch) Experiment\n{'='*70}")
...
ax.set_title("ANOVA + Boruta + MLP 2-View Classifier & Ensemble Performance")
ax.set_xticks(x)
ax.set_xticklabels(all_names)
ax.set_ylim(0, 1.05)
ax.legend(loc="lower right")
plt.tight_layout()
plot_file = os.path.join(OUT_DIR, "Kaggle_2view_Boruta_MLP_Ensemble.png")
...
cm_plot_file = os.path.join(OUT_DIR, "Kaggle_2view_Boruta_MLP_KNN_Confusion_Matrix.png")
plt.tight_layout()
...
ax.legend(loc="lower right")
roc_plot_file = os.path.join(OUT_DIR, "Kaggle_2view_Boruta_MLP_ROC_Curves.png")
plt.tight_layout()
